...
from app.contracts import AgentAskRequest, AgentReportSummaryRequest, ForecastRequest, PDFRenderRequest, PredictRequest
...
def create_app(settings: Settings) -> Robyn:
    ...
    @app.post("/internal/agent/v1/report-summary")
    def agent_report_summary(request: Request):
        def run() -> dict[str, Any]:
            payload = load_request_json(request)
            summary_request = AgentReportSummaryRequest.from_dict(payload)
            return agent_service.summarize_report(summary_request)

        return _handle_request(run)

    @app.post("/internal/agent/v1/render-pdf")
    def agent_render_pdf(request: Request):
        def run() -> dict[str, Any]:
            payload = load_request_json(request)
            pdf_request = PDFRenderRequest.from_dict(payload)
            return pdf_service.render_pdf(pdf_request)
