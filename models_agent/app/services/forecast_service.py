...
class ForecastService:
    ...
    def forecast(self, request: ForecastRequest) -> dict[str, Any]:
        if request.model_type != "lstm":
            raise ValidationError("当前预测接口仅支持 lstm")
        ...
        try:
            predictions = predict_single_sample(
                aggregate=[point.aggregate for point in recent_series],
                active_appliance_count=[point.active_appliance_count for point in recent_series],
                burst_event_count=[point.burst_event_count for point in recent_series],
