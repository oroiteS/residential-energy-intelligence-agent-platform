"""LSTM 直接预测任务的数据集与 LightningDataModule。"""
...
except ImportError:  # 兼容 `python forecast/lstm/dataloader.py`
    import importlib
...
class ForecastDataset(Dataset[dict[str, torch.Tensor]]):
    ...
    每个样本包含历史序列特征、未来日历特征、静态统计特征、标准化目标值
    以及未标准化真实值。训练使用 `target`，评估和反归一化使用 `actual`。
    """
...
class LSTMForecastDataModule(L.LightningDataModule):
    ...
    DataModule 负责读取配置指定的数据集、按时间切分训练/验证/测试集合、
    在训练集上拟合标准化器，并向 Lightning Trainer 提供 DataLoader。
    """
    ...
    def setup(self, stage: str | None = None) -> None:
        """读取数据、切分集合并构建 Dataset。

        Lightning 会在 fit/test/predict 阶段调用 setup；
        如果数据集已经创建过，则直接返回，避免重复读取和重复拟合标准化器。
        """
        ...
        # 标准化器只在训练集上拟合，然后应用到验证集和测试集。
        # 这是为了避免验证/测试分布信息泄漏到训练过程。
    ...
    def _make_datasets(
        ...
        *,
        ...
        # sequence 是三维数组，需要把“样本数 × 天数”展平成二维后拟合标准化器。
    ...
    def target_scaler_state(self) -> tuple[np.ndarray, np.ndarray]:
        """导出目标标准化参数，供模型 checkpoint 保存和推理反标准化使用。"""
