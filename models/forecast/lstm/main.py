...
except ImportError:  # 兼容 `python forecast/lstm/main.py`
    from test import test
    from train import train


def build_parser() -> argparse.ArgumentParser:
    """构建 LSTM Direct 的 train/test 子命令。"""

    parser = argparse.ArgumentParser(description="LSTM 直接预测任务")
    subparsers = parser.add_subparsers(dest="command")

    train_parser = subparsers.add_parser("train", help="训练 LSTM 直接预测模型")
    ...
    test_parser = subparsers.add_parser("test", help="测试 LSTM 直接预测模型")
...
def parse_test_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="测试 LSTM 直接预测模型")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent / "config" / "default.yaml",
