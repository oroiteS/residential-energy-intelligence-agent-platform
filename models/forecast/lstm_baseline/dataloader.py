"""LSTM 残差 baseline 任务的数据集与 LightningDataModule。"""
...
try:
    ...
    from .data import build_feature_spec, build_raw_arrays, get_baseline_predictions, time_split, validate_columns
except ImportError:  # 兼容 `python forecast/lstm_baseline/dataloader.py`
    import importlib
    ...
    get_baseline_predictions = getattr(_data, "get_baseline_predictions")
...
class ForecastDataset(Dataset[dict[str, torch.Tensor]]):
    ...
    `residual_target` 是真实 21 维目标减去 XGBoost baseline 后的残差；
    `baseline` 和 `actual` 用于模型输出还原及最终评估。
    """
...
class LSTMForecastDataModule(L.LightningDataModule):
    """LSTM 残差 baseline 任务的数据组织入口。

    与 Direct 数据模块不同，这里还会加载 XGBoost baseline 预测，
    并把真实目标转换为 residual = actual - baseline。
    """
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        # 前 7 个目标为总用电，XGBoost 直接预测；后 14 个峰/谷由 ratio 推导。
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        # train/valid/test 分别使用对应集合的 XGBoost baseline。
        # 残差模型只学习 baseline 没有解释掉的部分。
        ...
        *,
        ...
        def transform(
            ...
            return (
                ...
                cast(np.ndarray, target_scaler.transform(residual)),
                actual,
                baseline,
            )
        ...
        def dataset(raw: tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]) -> ForecastDataset:
