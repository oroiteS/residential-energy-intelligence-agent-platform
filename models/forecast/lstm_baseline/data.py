...
def predict_xgboost_baseline(
    ...
    *,
    ...
    baseline 模型只预测总用电量，后续再通过历史峰时占比扩展到峰/谷目标。
    """
...
def get_baseline_predictions(
    ...
    *,
    ...
    如果启用缓存，会直接读取 baseline_predictions.npz；
    否则加载 XGBoost 模型重新预测并写入缓存。
    """
...
def expand_baseline_to_21(xgboost_total: np.ndarray, hist_peak_ratio: np.ndarray) -> np.ndarray:
    ...
    hist_peak_ratio 来自历史 30 天峰时占比中位数；
    peak = total * ratio，valley = total * (1 - ratio)。
    """
...
def build_raw_arrays(
    ...
    # 残差模型学习的是“真实值 - XGBoost baseline”。
    # 最终预测时再把神经网络输出的残差加回 baseline。
