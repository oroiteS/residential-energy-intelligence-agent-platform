...
except ImportError:  # 兼容 `python forecast/transformer/main.py`
    from test import test
    from train import train


def build_parser() -> argparse.ArgumentParser:
    """构建 Transformer Direct 的 train/test 子命令。"""

    parser = argparse.ArgumentParser(description="Transformer 直接预测任务")
    subparsers = parser.add_subparsers(dest="command")

    train_parser = subparsers.add_parser("train", help="训练 Transformer 直接预测模型")
    ...
    test_parser = subparsers.add_parser("test", help="测试 Transformer 直接预测模型")
...
def parse_train_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="训练 Transformer 直接预测模型")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent / "config" / "default.yaml",
...
def parse_test_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="测试 Transformer 直接预测模型")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent / "config" / "default.yaml",
