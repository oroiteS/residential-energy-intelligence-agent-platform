#!/usr/bin/env python3
"""PyTorch Lightning Transformer 直接预测测试入口。"""
...
def test(config_path: Path, checkpoint_path: Path | None = None) -> None:
    ...
    model = TransformerDirectForecaster.load_from_checkpoint(ckpt, weights_only=False)
    model.eval()
    ...
    if total_rows:
        print("\n总用电预测指标：")
        for r in total_rows:
            print(f"  {r['target']}: RMSE={r['rmse']:.4f}, MAE={r['mae']:.4f}, MAPE={r['mape']:.2f}%")
    if peak_rows:
        print("\n峰时用电预测指标：")
        for r in peak_rows:
            print(f"  {r['target']}: RMSE={r['rmse']:.4f}, MAE={r['mae']:.4f}, MAPE={r['mape']:.2f}%")
