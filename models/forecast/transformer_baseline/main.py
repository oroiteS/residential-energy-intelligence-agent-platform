#!/usr/bin/env python3
"""Transformer 残差 baseline 任务命令行入口。"""
...
except ImportError:  # 兼容 `python forecast/transformer_baseline/main.py`
    from test import test
    from train import train


def build_parser() -> argparse.ArgumentParser:
    """构建 Transformer 残差 baseline 的 train/test 子命令。"""

    parser = argparse.ArgumentParser(description="Transformer 残差 baseline 任务")
    subparsers = parser.add_subparsers(dest="command")

    train_parser = subparsers.add_parser("train", help="训练 Transformer 残差预测模型")
    ...
    test_parser = subparsers.add_parser("test", help="测试 Transformer 残差预测模型")
...
def parse_test_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="测试 Transformer 残差预测模型")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent / "config" / "default.yaml",
