...
from classification.TCN.config import DEFAULT_CONFIG_PATH
from classification.TCN.predict import main as predict_main
from classification.TCN.test import main as test_main
from classification.TCN.train import main as train_main


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="TCN 分类任务统一入口，可选择训练模式或测试模式。",
        epilog=(
            "常用命令示例:\n"
            ...
            "  python classification/TCN/main.py train --config classification/TCN/configs/config.yaml"
        ),
    ...
    train_parser = subparsers.add_parser(
        "train",
        help="训练 TCN 分类模型",
        description="读取 yaml 配置，训练 TCN 四分类模型并保存最优权重。",
    )
